...
# ----------------------------------------------------------------------
# 3. DYNAMIC DATA FILTERING PROCESS (ปรับแก้ท่อส่งข้อมูลใหม่ให้ไหลตามกันทั้งหน้า)
# ----------------------------------------------------------------------
# กรองข้อมูลระดับรายสวนก่อนเสมอเพื่อให้สอดคล้องกับ checkbox ฟีเจอร์
...
# สร้างข้อมูลระดับเขตขึ้นมาใหม่โดยการ Group ตัวผลลัพธ์รายสวนจริง (ทำให้เปลี่ยนตาม Filter ทุกตัวแน่นอน)
...
# ผูกข้อมูลประชากรกลับเข้าไปเพื่อหาอัตราส่วนสถิติ
...
df_dist_summary["Visitor_Density_Ratio"] = df_dist_summary["Monthly_Visitors"] / df_dist_summary["Population"]


# คัดสรรโครงสร้างข้อมูลต้นทางเพื่อนำไปป้อนให้ชาร์ตและ KPI
if selected_district == "ทั้งหมด":
    y_axis_col = "District"
...
# คำนวณสถิติภาพรวม (KPI Cards) ให้สรุปยอดแบบ Dynamic สัมพันธ์กัน
...
### ส่วนที่ 2: กราฟขนาดพื้นที่รวม (เดี่ยว แถวยาวเต็มตา เปลี่ยนตามฟิลเตอร์)
...
if not df_chart_data.empty:
    ...
    fig_area = px.bar(
        ...
        y=y_axis_col,
        orientation='h',
        text=df_sorted_area["Chart_Area"].apply(lambda x: f" {x:,} ตร.ม."),
        color="Chart_Area",
    ...
    st.info("ไม่พบข้อมูลพื้นที่สำหรับเงื่อนไขการกรองนี้")

st.markdown("---")


### ส่วนที่ 3: กราฟสถิติจำนวนผู้ใช้งานจริง (เดี่ยว แถวยาวเต็มตา เปลี่ยนตามฟิลเตอร์)
...
if not df_chart_data.empty:
    ...
    fig_visitors = px.bar(
        ...
        y=y_axis_col,
        orientation='h',
        text=df_sorted_visitors["Chart_Visitors"].apply(lambda x: f" {x:,} คน/เดือน"),
