...
st.title("🌲 Park Finder & Air Quality Monitor")
...
def refresh_and_get_pm25():
    ...
    if air_data and 'stations' in air_data:
        for station in air_data['stations']:
            try:
                # 1. เจาะเข้าไปที่ชั้น AQILast -> PM25 -> value ตาม JSON จริงของ MongoDB
                ...
                # 2. เก็บเข้า DataFrame โดยใช้ lat และ long (ตัวพิมพ์เล็ก) ตาม JSON ดิบ
                stations_list.append({
                    'station_name': station.get('nameTH'),
                    'station_lat': float(station.get('lat')),
                    'station_lon': float(station.get('long')), # แก้จาก lng เป็น long
...
if not df_stations_pm25.empty and not df_parks_merged.empty:
    # 🌟 ฝั่งสวนใช้ตัวพิมพ์ใหญ่เป๊ะๆ ตาม Snowflake คือ LAT และ LNG
    park_coords = df_parks_merged[['LAT', 'LNG']].values
    
    # 🌟 ฝั่งสถานีฝุ่นอ้างอิงตามคีย์ที่เราตั้งไว้ใน DataFrame ของข้อ 1
